...
public class DomicilioDAOSQL implements DomicilioDAO {
	...
	private static final String selectForParameter = "SELECT * FROM domicilios where Calle=?,Altura=?,piso=?,departamento=?,localidadId=? ";
		...
		try {
			...
			while (resultSet.next()) {
				domicilios.add(new DomicilioDTO(
						resultSet.getInt("idDomicilio"), resultSet
								.getString("Calle"), resultSet
								.getString("Altura"), resultSet
								.getString("piso"),
						resultSet.getString("dpto"), resultSet
